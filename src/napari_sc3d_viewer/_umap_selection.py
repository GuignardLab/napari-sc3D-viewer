...
class UmapSelection():
    ...
    def show_cells(self, event):
        ...
        if event:
            ...
            if sum(len(s.ind) for s in self.selectors)==0:
                ...
                for i, (ax_hist, (gene_hist, maxi)) in enumerate(self.ax_hists):
                ...
                for s in self.selectors:
                    if len(s.ind)!=0:
                        ...
                        self.ax_G.set_title(f'Gene: {self.gene.value} ({nb} cells '
    ...
    def build_figure(self):
        ...
        pts_G = self.ax_G.scatter(*pos_cells[self.sorted_vals, :].T, marker='.',
        ...
        for j, (ax_hist, _) in enumerate(self.ax_hists):
            ...
            ax_hist.set_title(f'{gene_hist} distribution')
            ax_hist.set_xlim(0, max(.001, self.maximums[gene_id]))
        ...
        if self.tissues.value:
            ...
            pts_T = ax_T.scatter(*pos_cells[self.sorted_vals, :].T, marker='.',
            ...
            tissues_found = set([self.embryo.tissue[c] for c in self.corres_to_mask])
