...
from magicgui.widgets import FileEdit, LineEdit, Container, Label, Select
...
def display_embryo(viewer, embryo):
    ...
    #       labels=False)
    def select_tissues(self, viewer: Viewer, tissues):
    ...
    def __init__(self, viewer, embryo):
        ...
        self.tissues_to_plot = [t for t in tissues_to_plot if t in embryo.all_tissues]
        ...
        shown = [self.embryo.tissue[c] in tissues_to_plot for c in cells]
        if not any(shown):
        ...
        all_tissues = [self.embryo.corres_tissue.get(t, f'{t}')
                       for t in self.embryo.all_tissues]

        select_tissues = Select(choices=all_tissues,
                                value=[embryo.corres_tissue.get(t, f'{t}')
                                       for t in tissues_to_plot],
                                call_button='Select tissues')
        
        self.viewer.window.add_dock_widget(select_tissues)
